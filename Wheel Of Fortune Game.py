...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        ...
        if(self.prizeMoney > VOWEL_COST):
            ...
            for x in LETTERS:
                if x not in guessed and x not in VOWELS:
                    allowed.append(x)
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        allowed = []
        allowed = self.getPossibleLetters(guessed)
        if len(allowed) == 0:
            return 'pass'
        num = self.smartCoinFlip()
        
        if num == True:
            for x in range(len(SORTED_FREQUENCIES)):
                if SORTED_FREQUENCIES[x] in allowed:
                    return SORTED_FREQUENCIES[x]
